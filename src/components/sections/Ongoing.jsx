...
export default function Ongoing() {
  return (
    <section id="ongoing" className="relative bg-paper-soft py-28">
      <Container>
        <Reveal>
        ...
        <div className="mt-14 grid gap-12 lg:grid-cols-2 lg:items-start">
            ...
            {ongoingProjects.map((p, i) => (
              <Reveal key={p.client} delay={i * 0.1}>
                <div className="relative mb-10 last:mb-0">
                  <span className="absolute -left-[41px] top-1 flex h-6 w-6 items-center justify-center rounded-full bg-brand-500/15 ring-4 ring-paper-soft">
                    <HardHat size={12} className="text-brand-600" />
                  </span>
                  <h4 className="font-display text-lg font-semibold text-navy-900">{p.client}</h4>
                  <p className="mt-1 text-sm text-slate-500">{p.project}</p>
                </div>
              </Reveal>
            ))}
          </div>
          ...
          <Reveal delay={0.1}>
            <div className="relative aspect-[4/3] overflow-hidden rounded-2xl border border-line shadow-sm">
              <img
