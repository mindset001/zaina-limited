...
export default function Culture() {
  return (
    <section id="culture" className="relative bg-paper-soft py-28">
      <Container>
            ...
            {onePrinciples.map((p, i) => {
              ...
              return (
                <Reveal key={p.title} delay={i * 0.06}>
                  <div className="flex h-full flex-col items-start rounded-xl border border-line bg-white p-5 shadow-sm">
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-brand-500/10">
                      <Icon size={18} className="text-brand-600" />
                    </div>
                    <h4 className="mt-4 font-semibold text-navy-900">{p.title}</h4>
